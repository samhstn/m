...
/**
 * Exposes method `shellString` for generating a 'git grep ...' string to be to be executed in bash.
 */
class Mg {
  constructor(args, ops = {archive: false}) {
    this._readPattern = this._readPattern.bind(this);
    ...
    this.pattern = this._readPattern(args);
  ...
  get shellString() {
    ...
    return `git grep ${flags.join(' ')} '${safePattern}'${dirString}${excludePatternString}${withNumbersString}`;
  }
}
